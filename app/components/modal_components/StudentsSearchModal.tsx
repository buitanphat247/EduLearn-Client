...
import type { StudentItem } from "../students_components/types";

interface StudentsSearchModalProps {
  open: boolean;
  onClose: () => void;
  data: StudentItem[];
}

export default function StudentsSearchModal({ open, onClose, data }: StudentsSearchModalProps) {
  ...
  const handleItemClick = (item: StudentItem) => {
    router.push(`/admin/students/${item.key}`);
    onClose();
    setSearchQuery("");
  };

  const handleViewClick = (e: React.MouseEvent, item: StudentItem) => {
    e.stopPropagation();
    router.push(`/admin/students/${item.key}`);
    onClose();
    setSearchQuery("");
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3 text-sm text-gray-600">
                          <span className="font-mono text-xs bg-gray-100 px-2 py-0.5 rounded">{item.studentId}</span>
                          <span className="font-mono text-xs bg-gray-100 px-2 py-0.5 rounded">{item.class}</span>
                          <span>{item.email}</span>
                          <Tag
                            className="px-2 py-0.5 rounded-md text-xs"
                            color={item.status === "Đang học" ? "green" : item.status === "Tạm nghỉ" ? "orange" : "blue"}
                          >
                            {item.status}
                          </Tag>
                        </div>
