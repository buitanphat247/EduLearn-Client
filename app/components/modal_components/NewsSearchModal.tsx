...
import type { NewsItem } from "../news_components/types";
...
export default function NewsSearchModal({ open, onClose, data }: NewsSearchModalProps) {
  ...
  const filteredData = data.filter((item) => {
    ...
    return item.title.toLowerCase().includes(query) || item.category.toLowerCase().includes(query) || item.status.toLowerCase().includes(query);
  });

  const handleItemClick = (item: NewsItem) => {
    router.push(`/admin/news/handle/${item.key}`);
    onClose();
    setSearchQuery("");
  };

  const handleViewClick = (e: React.MouseEvent, item: NewsItem) => {
    e.stopPropagation();
    router.push(`/news/${item.key}`);
    onClose();
    setSearchQuery("");
  };
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <h3 className="font-semibold text-gray-800 group-hover:text-blue-600 transition-colors mb-1.5">{item.title}</h3>
                        <div className="flex items-center gap-3 text-sm text-gray-600">
                          <Tag className="px-2 py-0.5 rounded-md text-xs" color={item.category === "Tin tức" ? "blue" : "purple"}>
                            {item.category}
                          </Tag>
                          <Tag className="px-2 py-0.5 rounded-md text-xs" color={item.status === "Đã xuất bản" ? "green" : "orange"}>
                            {item.status}
                          </Tag>
                          <span>{item.date}</span>
                        </div>
