...
import type { ClassItem } from "../classes_components/types";

interface ClassesSearchModalProps {
  open: boolean;
  onClose: () => void;
  data: ClassItem[];
}

export default function ClassesSearchModal({ open, onClose, data }: ClassesSearchModalProps) {
  ...
  const handleItemClick = (item: ClassItem) => {
    router.push(`/admin/classes/handle/${item.key}`);
    onClose();
    setSearchQuery("");
  };

  const handleEditClick = (e: React.MouseEvent, item: ClassItem) => {
    e.stopPropagation();
    router.push(`/admin/classes/handle/${item.key}`);
    onClose();
    setSearchQuery("");
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3 text-sm text-gray-600">
                          ...
                          <Tag className="px-2 py-0.5 rounded-md text-xs" color={item.status === "Đang hoạt động" ? "green" : "orange"}>
                            {item.status}
                          </Tag>
