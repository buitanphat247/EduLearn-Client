...
import type { ExerciseItem } from "../exercises_components/types";

interface ExercisesSearchModalProps {
  open: boolean;
  onClose: () => void;
  data: ExerciseItem[];
}

export default function ExercisesSearchModal({ open, onClose, data }: ExercisesSearchModalProps) {
  ...
  const handleItemClick = (item: ExerciseItem) => {
    router.push(`/admin/exercises/${item.key}`);
    onClose();
    setSearchQuery("");
  };

  const handleViewClick = (e: React.MouseEvent, item: ExerciseItem) => {
    e.stopPropagation();
    router.push(`/admin/exercises/${item.key}`);
    onClose();
    setSearchQuery("");
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3 text-sm text-gray-600">
                          ...
                          <span>{item.date}</span>
                          <Tag className="px-2 py-0.5 rounded-md text-xs" color={item.status === "Đang mở" ? "green" : "orange"}>
                            {item.status}
                          </Tag>
                        </div>
                      </div>
