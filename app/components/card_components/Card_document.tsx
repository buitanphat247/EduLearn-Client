...
export default function CardDocument({ title, grade, subject, updateDate, author, downloads, type, onPreview }: DocumentCardProps) {
    ...
    switch (type) {
      ...
      case "pdf":
        return {
          ...
          iconBg: "bg-linear-to-br from-red-500 to-pink-500",
  ...
  return (
    ...
    >
      ...
      <div className={`absolute inset-0 bg-linear-to-br ${config.gradient} opacity-0 group-hover:opacity-5 transition-opacity duration-300`}></div>

      <div className="relative z-10">
        {/* Icon Section */}
        <div className="flex items-start gap-4 mb-4">
          <div
            className={`shrink-0 w-16 h-16 rounded-xl flex items-center justify-center ${config.iconBg} shadow-md group-hover:shadow-lg transition-all duration-300`}
          >
            {type === "word" ? (
