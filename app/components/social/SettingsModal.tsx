...
export default function SettingsModal({ open, onClose }: SettingsModalProps) {
  ...
  const renderContent = () => {
    switch (activeSection) {
      case "general":
        return (
          <div className="space-y-6">
            <h3 className="text-lg font-bold text-white mb-4">Danh bạ</h3>
            <div className="space-y-4">
              <p className="text-slate-400 text-sm mb-2">Danh sách bạn bè được hiển thị trong danh bạ</p>
              <div className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-800/50 cursor-pointer transition-colors">
                <span className="text-slate-200">Hiển thị tất cả bạn bè</span>
                <div className="w-5 h-5 rounded-full border border-slate-600"></div>
              </div>
              <div className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-800/50 cursor-pointer transition-colors">
                <span className="text-slate-200">Chỉ hiển thị bạn bè đang sử dụng Zalo</span>
            ...
            <div className="flex items-center justify-between p-3 bg-slate-800 rounded-xl">
              <span className="text-slate-200">Thay đổi ngôn ngữ</span>
              <Select
                defaultValue="vi"
                style={{ width: 120 }}
            ...
            <h3 className="text-lg font-bold text-white mb-4">Khởi động & ghi nhớ tài khoản</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-slate-200">Khởi động Zalo khi mở máy</span>
                <Switch defaultChecked />
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-200">Ghi nhớ tài khoản đăng nhập</span>
                <Switch defaultChecked />
              </div>
            </div>
          </div>
      ...
      case "theme":
        return (
          <div className="space-y-6">
            <h3 className="text-lg font-bold text-white mb-4">Giao diện</h3>
            <div className="flex items-center justify-between p-4 bg-slate-800 rounded-xl">
              <div className="flex items-center gap-3">
                <AppstoreOutlined className="text-purple-500 text-xl" />
                <div>
                  <p className="font-medium text-slate-200">Chế độ tối</p>
                  <p className="text-xs text-slate-400">Giao diện tối giúp bảo vệ mắt</p>
                </div>
              </div>
              <Switch defaultChecked />
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between p-4 bg-slate-800 rounded-xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-slate-200">Âm thanh thông báo</p>
                </div>
              </div>
              <Switch defaultChecked />
            </div>
  ...
  return (
    <Modal
      ...
      modalRender={(modal) => (
        <div className="bg-[#0f172a] rounded-2xl overflow-hidden border border-slate-700/50 shadow-2xl shadow-black/50 p-0 pointer-events-auto">
          {React.cloneElement(modal as any, { style: { ...(modal as any).props?.style, padding: 0 } })}
          <div className="flex h-[600px] text-slate-200">
            {/* Sidebar */}
            <div className="w-[280px] bg-[#1e293b]/50 border-r border-slate-700/30 flex flex-col">
              <div className="p-5 border-b border-slate-700/30">
                <h2 className="text-xl font-bold text-white m-0">Cài đặt</h2>
              </div>
            ...
            <div className="flex-1 flex flex-col min-w-0 bg-[#0f172a]">
              {/* Header */}
              <div className="h-[69px] border-b border-slate-700/30 flex items-center justify-end px-6">
                <button
                  onClick={onClose}
                  className="w-8 h-8 rounded-full hover:bg-slate-800 flex items-center justify-center text-slate-400 hover:text-white transition-colors cursor-pointer border-none bg-transparent"
                >
