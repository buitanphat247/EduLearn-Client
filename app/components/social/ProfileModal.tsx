        ...
        <div className="px-6 pb-8 -mt-12 flex flex-col items-center">
          ...
          <div className="relative mb-4">
            <div className="rounded-full p-1 bg-white dark:bg-[#0f172a]">
              {user.avatar ? (
                <Avatar src={user.avatar} size={100} className="border-4 border-white dark:border-[#0f172a]" />
              ) : (
                <div className="w-[100px] h-[100px] rounded-full bg-linear-to-br from-indigo-500 to-cyan-500 flex items-center justify-center text-white text-4xl font-bold border-4 border-white dark:border-[#0f172a]">
                  {(user.fullname?.[0] || user.username?.[0] || "U").toUpperCase()}
                </div>
              )}
            </div>
            <button className="absolute bottom-1 right-1 bg-white dark:bg-slate-800 text-gray-700 dark:text-white p-2 rounded-full border border-gray-200 dark:border-slate-700 hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors shadow-lg">
              <CameraOutlined />
            </button>
          </div>
          ...
          <p className="text-gray-500 dark:text-slate-400 mb-2">@{user.username}</p>
          ...
          {/* Stats / Quick Info */}
          <div className="grid grid-cols-3 gap-4 w-full mb-8 border-y border-gray-200 dark:border-slate-800 py-4">
            <div className="flex flex-col items-center">
              <span className="text-gray-900 dark:text-white font-bold text-lg">{friendCount}</span>
              <span className="text-xs text-gray-400 dark:text-slate-500 uppercase tracking-wider">Bạn bè</span>
            </div>
            <div className="flex flex-col items-center border-l border-gray-200 dark:border-slate-800">
              <span className="text-gray-900 dark:text-white font-bold text-lg">{groupCount}</span>
              <span className="text-xs text-gray-400 dark:text-slate-500 uppercase tracking-wider">Nhóm</span>
            </div>
            <div className="flex flex-col items-center border-l border-gray-200 dark:border-slate-800">
              <span className="text-green-500 font-bold text-lg">Online</span>
              <span className="text-xs text-gray-400 dark:text-slate-500 uppercase tracking-wider">Trạng thái</span>
            </div>
          </div>
