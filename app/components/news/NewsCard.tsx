...
export default function NewsCard({ 
  ...
  const containerClasses = `h-full bg-[#1e293b] rounded-[20px] transition-all duration-300 flex flex-col relative overflow-hidden group-hover:shadow-2xl group-hover:shadow-blue-900/10 hover:-translate-y-1 ${
      accentColor 
        ? `border-l-[6px] ${accentColor} border-t border-r border-b border-slate-800` 
    ...
    <div className={containerClasses}>
        ...
        {image && !isEvent && (
           <div className="relative aspect-[16/10] overflow-hidden">
              <span className="absolute top-4 left-4 z-10 px-3 py-1 bg-blue-500/20 text-blue-400 text-xs font-bold uppercase rounded-md backdrop-blur-md border border-blue-500/20">
                 {category || 'Tin tức'}
              </span>
        ...
        <div className="p-6 flex flex-col flex-1">
           ...
           {isEvent && (
              <div className="flex justify-between items-start mb-4">
                 <span className={`px-3 py-1 text-xs font-bold uppercase rounded-full border ${
                    accentColor?.includes('blue') ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' :
                    accentColor?.includes('green') || accentColor?.includes('emerald') ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
           ...
           {/* Title */}
           <h3 className={`text-lg font-bold text-white mb-3 line-clamp-2 group-hover:text-blue-400 transition-colors ${isEvent ? 'text-xl mb-6' : ''}`}>
              {title}
           </h3>
           ...
           <div className="mt-auto pt-5 border-t border-slate-700/50 flex items-center justify-between">
              <span className="text-slate-300 text-sm font-semibold group-hover:text-blue-400 transition-colors">
                 {isEvent ? 'Xem chi tiết' : 'Đọc chi tiết'}
              </span>
              <div className="w-8 h-8 rounded-full bg-slate-700/50 flex items-center justify-center text-slate-400 group-hover:bg-blue-600 group-hover:text-white transition-all transform group-hover:translate-x-1">
                 <ArrowRightOutlined />
              </div>
           </div>
        </div>
