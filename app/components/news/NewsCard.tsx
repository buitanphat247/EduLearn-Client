...
export default function NewsCard({ 
  ...
  const containerClasses = `h-full bg-white dark:bg-[#1e293b] rounded-[20px] transition-all duration-300 flex flex-col relative overflow-hidden group-hover:shadow-2xl group-hover:shadow-blue-900/10 hover:-translate-y-1 ${
      accentColor 
           ...
           {isEvent && (
              <div className="flex justify-between items-start mb-4">
                 <span className={`px-3 py-1 text-xs font-bold uppercase rounded-full border ${
                    accentColor?.includes('blue') ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20' :
