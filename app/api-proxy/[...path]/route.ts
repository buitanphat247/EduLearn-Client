...
async function handleRequest(request: NextRequest, method: string) {
  ...
  if (!rateLimitResult.success) {
    return responseHandler.createRateLimitResponse(rateLimitResult.limit, rateLimitResult.remaining, rateLimitResult.reset);
  }

  // 2. SSRF Protection - Validate path and URL
  const validation = ssrfProtection.validateRequest(request.url, backendUrl);
  if (!validation.valid) {
    return responseHandler.createErrorResponse(validation.error || "Invalid request", 403);
  }
  ...
  try {
    ...
    const requestConfig = await requestHandler.buildRequestConfig(request, method, targetUrl!);
    const response = await requestHandler.executeRequest(requestConfig);

    // 6. Process response
    const processedResponse = await responseHandler.processResponse(response, "MISS");

    // 7. Cache successful GET responses
    if (method === "GET" && response.status === 200) {
      const url = new URL(request.url);
      proxyCache.set(method, path!, url.search, processedResponse.data, processedResponse.headers, userId);
    }

    // Note: 401 errors are handled silently - use browser DevTools Network tab for debugging
